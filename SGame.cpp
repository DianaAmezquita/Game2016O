...
void CSGame::OnEntry(void)
{
	...
	auto fx = MAIN->m_pSndManager->LoadSoundFx(L"..\\Assets\\ScreenMusic.wav", SND_GAME);
	if (fx)
	{
	...
	fflush(stdout);
	m_pSndBackground = MAIN->m_pSndManager->LoadSoundFx(L"..\\Assets\\ScreenMusic.wav", SND_GAME);
	if (m_pSndBackground)
	{
	...
	if (EVENT_WIN32 == pEvent->m_ulEventType)
	{
		...
		case WM_TIMER:
			if (2 == pWin32->m_wParam)
			{
				...
				MAIN->m_pSndManager->PlayFx(SND_GAME, 1, 0, 0.3);
			}
			break;
		case WM_CLOSE:
